    ...
    const loadCollections = async () => {
      ...
      try {
        ...
        const moreCollections = [
          ...
          {
            id: "pixel-landscapes-01",  // Changed ID to make it unique
          ...
          },
          {
            ...
            image: "/placeholder.svg?height=100&width=100",
            items: 70,
            floorPrice: "0.15 tFIL",
          },
        ]

        // Make sure there are no duplicate IDs
        const combinedCollections = [...data];
        
        // Only add collections from moreCollections if their IDs don't already exist
        moreCollections.forEach(collection => {
          if (!combinedCollections.some(existing => existing.id === collection.id)) {
            combinedCollections.push(collection);
          }
        });
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 relative z-10">
            {sortedCollections.map((collection, index) => (
              <Link key={`${collection.id}-${index}`} href={`/collection/${collection.id}`}>
                <div
